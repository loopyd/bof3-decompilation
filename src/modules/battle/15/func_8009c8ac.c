...
u8 func_8009c8ac(u16 required_mask) {
  ...
  if (BATTLE_PANEL_RULE_PASS_KIND == 4u) {
    if ((BATTLE_SELECTION_KIND_MASK(BATTLE_PANEL_RULE_PASS_SELECTION) &
         required_mask) != 0u) {
      return 1u;
    }
  }
  ...
  panel_slot_kind = BATTLE_PANEL_SLOT_KIND(BATTLE_PANEL_RULE_PASS_SLOT);
  if ((BATTLE_PANEL_SLOT_MASK(panel_slot_kind) & (u8)required_mask) != 0u) {
    return 1u;
  }
