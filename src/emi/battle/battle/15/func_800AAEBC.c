...
void func_800AAEBC(s16 target_index, u8 battler_index) {
  ...
  MMIO8(0x801ec33bu + ((u32)action_slot * 0x78u)) = 1u;

  if (battler_index < 3u) {
    battler_data = (volatile u8*)(0x80145e90u + ((u32)battler_index * 0x140u));
    target_flags = MMIO8(0x80145fb0u + ((u32)battler_index * 0x140u));
  } else {
    enemy = battler_index - 3u;
    battler_data = (volatile u8*)(0x801eb2e8u + ((u32)enemy * 0x118u));
    target_flags = MMIO8(0x801eb72cu + ((u32)enemy * 0x118u));
  }

  if (target_index < 0) {
    target_index = -target_index;
    MMIO32(0x801ec390u + ((u32)action_slot * 0x78u)) = (s32)target_index;
    MMIO8(0x801ec357u + ((u32)action_slot * 0x78u)) = 1u;
  } else {
    MMIO32(0x801ec390u + ((u32)action_slot * 0x78u)) = (s32)target_index;
    MMIO8(0x801ec357u + ((u32)action_slot * 0x78u)) = 2u;
  }

  if (!(target_flags & 0x2u)) {
    MMIO8(0x801ec337u + ((u32)action_slot * 0x78u)) = 4u;
    return;
  }

  if (!(target_flags & 0x20u) && (target_flags & 0x8u)) {
    MMIO8(0x801ec357u + ((u32)action_slot * 0x78u)) = 1u;
    MMIO8(0x801ec337u + ((u32)action_slot * 0x78u)) = 2u;
    return;
  }

  MMIO8(0x801ec337u + ((u32)action_slot * 0x78u)) = 0u;
}
