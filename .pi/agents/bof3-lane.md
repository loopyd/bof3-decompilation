...
2. Run its inner-lane `workflowScript` with exactly one selector and a unique `RUN_KEY`, explicitly setting workflow-level `worktree:false` so every nested child remains in this managed lane worktree. Use sequential mutation/review phases; never write concurrently here.
