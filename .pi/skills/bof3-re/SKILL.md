...
Non-exact review returns 1–3 ranked variants with expected codegen; ledger ≤20 attempts, preserving best. Prefer evidence-backed, then labeled reversible semantics-preserving speculative C shapes. Stop only at exact, rejected semantics/types, approval/safety/external blocker, or ceiling. Retain coherent improvement with atomic `@status partial`/`@match`/`@residual`; revert only no-progress/semantic defects. Partial→exact review identifies the decisive experiment; parent records a generalizable playbook/lesson rule.
