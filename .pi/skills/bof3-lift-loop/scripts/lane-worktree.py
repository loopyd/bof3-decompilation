...
def record(args: argparse.Namespace) -> int:
    ...
    if not isinstance(transaction_id, str) or not transaction_id:
        raise SystemExit("ledger entry omitted transaction id")
    LEDGERS.mkdir(parents=True, exist_ok=True)
    path = ledger_path(args.selector)
    with path.open("a+") as output:
        fcntl.flock(output, fcntl.LOCK_EX)
        output.seek(0)
        entries = [json.loads(line) for line in output if line.strip()]
        matches = [
            item for item in entries if item.get("transaction_id") == transaction_id
        ]
        if matches and matches != [entry]:
            raise SystemExit("ledger transaction id collision")
        if not matches:
            output.write(json.dumps(entry, sort_keys=True) + "\n")
            output.flush()
            os.fsync(output.fileno())
            entries.append(entry)
    print(json.dumps({"recorded": not matches, "entries": len(entries)}))
    return 0


def ledger(args: argparse.Namespace) -> int:
    print(
        json.dumps({"selector": args.selector, "entries": read_ledger(args.selector)})
    )
    return 0
...
def git(*args: str, cwd: Path = ROOT, capture: bool = False) -> str:
    result = subprocess.run(
        ("git", *args), cwd=cwd, check=True, text=True, capture_output=capture
    )
...
def identity(
    key: str, selector: str, base: str, worktree: str, session_dir: str
) -> str:
    return hashlib.sha256(
        json.dumps([key, selector, base, worktree, session_dir]).encode()
    ).hexdigest()
...
def lane_state(key: str, *, check_head: bool = True) -> tuple[Path, Path, dict]:
    ...
    if (
        ...
        != {
            "cwd": str(expected),
            "worktree": False,
            "sessionDir": str(expected_session),
            "async": True,
        }
        ...
        or (
            check_head
            and state.get("base")
            != git("rev-parse", "HEAD", cwd=expected, capture=True)
        )
...
def create(args: argparse.Namespace) -> int:
    ...
    try:
        subprocess.run(
            ("python3", ".pi/scripts/bootstrap-bof3-lane.py"), cwd=worktree, check=True
        )
        ...
        state["identity"] = identity(
            args.key, args.selector, base, str(worktree), str(session_dir)
        )
...
def integrate(args: argparse.Namespace) -> int:
    ...
    with INTEGRATION_LOCK.open("w") as lock:
        ...
        forbidden = [
            ...
            if path.startswith(FORBIDDEN_PREFIXES) or Path(path).is_absolute()
        ]
        if forbidden:
            raise SystemExit("forbidden lane paths: " + ", ".join(forbidden))
        try:
            subprocess.run(
                ("git", "apply", "--check", str(patch)), cwd=ROOT, check=True
            )
