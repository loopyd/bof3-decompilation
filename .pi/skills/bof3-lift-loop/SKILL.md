---
name: bof3-lift-loop
description: Autonomously lifts a serial BOF3 batch to reviewed exact byte matches. Selects candidates, dispatches bounded executor/reviewer agents, and commits only user-authorized reviewed exact lifts. Use for a reviewed batch; use `/skill:bof3-re` for one hand-guided lift.
---
...
- One function/lane. Rendered workflow owns loop, 20-attempt ceiling, baseline checkpoint, restore, stops; do not duplicate. Iteration executors launch with `acceptance:false`: one queue is intentionally incomplete, so the workflow's reviewer/final gates decide continuation and retention. Every baseline, attempt, and rung transition is also appended to the parent-owned selector ledger under `out/lift-loop/experiment-ledgers/`; replacement lanes load it and exclude previously recorded variants. Removing a lane never removes this ledger.
- An attempt is a substantive investigation pass: reviewer queues at least three distinct evidence-backed experiments; executor runs the complete queue plus related safe variants before returning; reviewer independently reloads live evidence. Fewer than three distinct experiments blocks the next retry. The shared worktree carries discoveries forward; whenever an improvement or exact result becomes best, first persist `best-promoting` with its score/attempt/selector, then capture a unique complete checkpoint and atomically persist its score/leaf while clearing the pending intent. Restart verifies durable `best.json`: a matching promoted best is adopted before restore; malformed or mismatched promotion state blocks rather than restoring a stale prior best. Capture failure leaves the recoverable pending phase and prior best authoritative. Three consecutive non-improving queues automatically advance one rung: `clean-c` → `static-allocation` → `compiler-profile` (`bin/flag-search`) → one bounded `permuter` coordinator → `compiler-ceiling`. An improvement resets the stall count but does not move backward to an exhausted rung. Rung changes are ledger entries; reviewers must queue experiments for the active rung, not resume broad spelling churn. Final and interrupted recovery restore the persisted best checkpoint, never merely the baseline.
- Record a reusable decisive lever in the narrowest playbook/lesson before integration; function-only → `lesson: none`.
- Cleanup every retained exact/partial only with `retained-lift TARGET SELECTOR STATE [ROW...]`, where state is `exact|improved-partial` and rows are already prepared naming-audit/v3 IDs carried from the final result. Never dispatch generic `repair`. Exact requires fresh diff/bytes/symbols/Splat/relocation + review. Partial is spelling-only; preserve body/ABI/boundary/compiler and atomic status/match/residual, score ≥ best + audits/review. Failure restores pre-cleanup.
...
- Create with `python3 .pi/skills/bof3-lift-loop/scripts/lane-worktree.py create --key WAVE-LANE --selector SELECTOR`; it bootstraps executable modes, tools, `.venv`/`inputs`, compilers, disposable `out/`, and returns an absolute `session_dir`.
...
- After retained exact/improvement, the workflow runs `bof3-cleanup` and consolidation review. Accept explicit `pass` for exact and `pass|retain-improved-partial|retain-as-improved-partial` for a partial; warnings do not block a reviewed partial unless verdict is `block|needs-fix`. Then a host gate calls `lane-worktree.py integrate`: clean parent at unchanged lane base, export/digest/check/apply only non-ignored Git changes, `git diff --check`, commit the reviewed transaction, remove the lane. Generated ignored `build/`/`.pi-subagents/` state is expected in lanes and excluded, not a handoff failure. Any failure rolls parent changes back and preserves the lane. The workflow returns `integrated` only after commit + cleanup; otherwise close the mission `failed` with lane status/ledger summary. Reject failed capture, unexpected paths, `src/emi/`, absolute-path stubs, unapproved `INCLUDE_ASM`, cleanup/review rejection, or score regression.
...
`parallelism=1` remains the default and uses one managed worktree; higher values fan out isolated lane worktrees.
