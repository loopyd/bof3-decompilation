...
Freeze one fresh queue, then launch up to `parallelism` target-distinct `bof3-lane` children as independent top-level async workflows, each with `worktree:true` and a unique absolute project-owned `sessionDir`. Do not put lane orchestrators in one `runs.all`: current pi-subagents assigns them the same `run-0/session.jsonl`, causing `EEXIST`. Each managed worktree exists from the lane's first reverse attempt through terminal cleanup/final review; its lane orchestrator serializes nested writers/reviewers in that cwd. Requirements:

- Never place two live lanes from the same target together. Partition by distinct `TARGET`; defer collisions.
- Use native pi-subagents isolation and handoff only. Each lane first runs `.pi/scripts/bootstrap-bof3-lane.py` to restore executable modes, populated matching-tool submodules, ignored `.venv`/`inputs`, real (non-symlink) compiler files, and a lane-local reflink/copy of disposable `out/`; Git excludes this bootstrap state from native patch capture. No custom worktree manager, extension config mutation, or patch exporter.
- Top-level call per selector: `subagent({workflowScript:"return runs.run('lane',{agent:'bof3-lane',task:'Run SELECTOR...',worktree:true})", sessionDir:"/absolute/project/.pi-subagents/sessions/WAVE/LANE", async:true, ...})`. Launch calls back-to-back for asynchronous parallelism. The lane agent is an explicit bounded fanout orchestrator; nested reverse/review/cleanup children use `worktree:false` and one writer at a time.
