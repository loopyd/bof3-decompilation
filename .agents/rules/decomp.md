...
- Fixed-address RAM globals: declare `extern type DAT_xxxxx;` in `internal.h` with a `/* does: ... */` comment, and place via `SYMBOL_AT(DAT_xxxxx, 0x8XXXXXXX)` in `src/boot/symbols.c` — never `#define DAT_xxxxx VUxx(addr)`
