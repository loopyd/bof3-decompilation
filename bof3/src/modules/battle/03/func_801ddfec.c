  ...
  if ((arg0 & 0xffu) < 3u) {
    ...
    while ((index & 0xffu) < BATTLE_GLOBAL_BYTE_6323) {
      ...
      if (battler == (arg0 & 0xffu)) {
        if (BATTLE_LOCAL_BYTE_119(
                &BATTLE_LOCAL_WORK_ARRAY[battler]) != 5u) {
          return;
        }
        index =
            BATTLE_LOCAL_HALF_11A(&BATTLE_LOCAL_WORK_ARRAY[battler]);
        if ((index >> 8) != 0u) {
          return;
        }
        func_801dde7c(
            BATTLE_LOCAL_BYTE_122(&BATTLE_LOCAL_WORK_ARRAY[battler]),
            index);
        return;
      }
      index += 1u;
