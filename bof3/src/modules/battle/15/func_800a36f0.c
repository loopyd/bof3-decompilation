...
u16 func_800a36f0(u8 battler_index, u16 flags) {
  ...
  if (flags & 0x4000u) { status &= 0xbfffu; }
  if (flags & 0x80u)   { status &= 0xff7fu; }
  if (flags & 0x1u)    { status &= 0xfffeu; }
  if (flags & 0x2u)    { status &= 0xfffdu; }
  if (flags & 0x4u)    { status &= 0xfffbu; }
  if (flags & 0x40u)   { status &= 0xffbfu; }
  if (flags & 0x8u)    { status &= 0xfff7u; }
  if (flags & 0x10u)   { status &= 0xffefu; }
  if (flags & 0x800u)  { status &= 0xf7ffu; }
  ...
  if (battler_index < 3u) {
    ...
    ((void(*)(volatile u8*))0x80196718u)(player_base + ((u32)player * 0x140u) + 0x5e90u);
    ...
    enemy = battler_index - 3u;
    ((void(*)(volatile u8*))0x80196718u)((volatile u8*)(0x801eb2e8u + ((u32)enemy * 0x118u)));
    scratchpad_saved = REG32(0x1f800044u);
