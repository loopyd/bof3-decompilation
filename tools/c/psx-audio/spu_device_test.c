...
static int configure_voice(PsxSpu* spu, unsigned int voice) {
  return write_register(spu, PSX_SPU_MAIN_VOLUME_LEFT, 0x3fff) ||
         ...
         write_register(spu, voice_address(voice, PSX_SPU_VOICE_VOLUME_LEFT),
                        0x3fff) ||
         write_register(spu, voice_address(voice, PSX_SPU_VOICE_VOLUME_RIGHT),
                        0x3fff) ||
         write_register(spu, voice_address(voice, PSX_SPU_VOICE_PITCH),
                        0xffff) ||
         write_register(spu, voice_address(voice, PSX_SPU_VOICE_START_ADDRESS),
                        0) ||
...
static int test_pitch_cap(void) {
  ...
  if (!first || !second)
    goto done;
  if (upload_loop(first) || upload_loop(second) || configure_voice(first, 0) ||
      configure_voice(second, 0) ||
