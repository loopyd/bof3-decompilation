...
static int tone_matches(const VabHdr* vab, int program, int note) {
  int i;
  int matches = 0;

  for (i = 0; i < vab->tone_count; i++) {
    const VagAtr* tone = &vab->tones[i];
...
static void audit_samples(const uint8_t* vb, size_t vb_len, const VabHdr* vab,
  ...
  for (i = 0; i < vab->tone_count; i++) {
    ...
    if (tone->mode & 4)
      report->reverb_tones++;
    if (tone->vibrato_width || tone->vibrato_time || tone->portamento_width ||
        tone->portamento_time)
    ...
    for (block = tone->vag_offset + 16;
         block + 16 <= tone->vag_offset + tone->vag_size; block += 16) {
      if (vb[block + 1] & 1) {
        has_end = 1;
        break;
      }
...
static void audit_sequence(const SepSequence* sequence, const VabHdr* vab,
                           AudioAuditReport* report) {
  int programs[16] = {0};
  int i;
