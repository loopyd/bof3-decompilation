...
int flac_write_stereo(const char *path, const int16_t *pcm, int64_t frames, int rate)
{
    ...
    if (!FLAC__stream_encoder_set_channels(encoder, 2) ||
        ...
        !FLAC__stream_encoder_set_total_samples_estimate(encoder, (FLAC__uint64)frames) ||
        FLAC__stream_encoder_init_file(encoder, path, NULL, NULL) != FLAC__STREAM_ENCODER_INIT_STATUS_OK)
        goto free_buffer;
