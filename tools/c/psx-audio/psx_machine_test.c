...
int main(void) {
  ...
  failed = failed || psx_spu_write_count(spu) != 1 || writes[0].cycle != 3 ||
           writes[0].address != PSX_SPU_REGISTER_BASE ||
           writes[0].value != 0x1234 || psx_machine_pc(machine) != 0x80010010u;
  psx_machine_destroy(machine);
  psx_spu_destroy(spu);
  ...
  failed = psx_spu_write16(spu, PSX_SPU_TRANSFER_ADDRESS, 2, 10) != 0 ||
           ...
           psx_spu_ram(spu)[16] != 0x12 || psx_spu_ram(spu)[17] != 0x34 ||
           psx_spu_ram(spu)[18] != 0x56 || psx_spu_ram(spu)[19] != 0x78;
  failed = failed || psx_spu_write16(spu, PSX_SPU_CONTROL, 0x8000, 13) != 0 ||
           ...
           psx_spu_ram(spu)[20] != 0;
  {
    ...
    failed = failed ||
             ...
             psx_spu_ram(spu)[32] != 0xaa || psx_spu_ram(spu)[33] != 0xbb ||
             psx_spu_ram(spu)[34] != 0xcc || psx_spu_ram(spu)[35] != 0xdd;
  }
  psx_spu_destroy(spu);
  return failed;
}
