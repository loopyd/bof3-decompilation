...
static Psf1Status read_whole_file(const char *path, uint8_t **data, size_t *size)
{
    FILE *stream;
    long length;
    uint8_t *buffer;
...
static Psf1Status psf_file_read(const char *path, PsfFile *file)
{
    ...
    if (uncompress(file->program, &output_size, compressed, compressed_size) != Z_OK) {
...
static int tag_value(const char *tags, const char *name,
...
static Psf1Status load_chain(const char *path, Psf1Image *image,
                             int *entry_set, int *refresh_set, int depth)
{
    PsfFile file;
    Psf1Status status;
    char library[512];
    ...
    if (index > 0) {
        status = resolve_library_path(path, library, library_path,
                                      sizeof(library_path));
        if (status == PSF1_OK)
            status = load_chain(library_path, image, entry_set, refresh_set,
                                depth + 1);
        if (status != PSF1_OK)
            goto done;
    }
...
static int exe_region_refresh_rate(const PsfFile *file)
{
    if (file->program_size > 0x4c + 56 &&
        memcmp(file->program + 0x4c, "Sony Computer Entertainment Inc. for Europe", 43) == 0)
        return 50;
    return 60;
}
...
{
    switch (status) {
    case PSF1_OK: return "ok";
    case PSF1_ERROR_ARGUMENT: return "invalid argument";
    case PSF1_ERROR_IO: return "I/O error";
    case PSF1_ERROR_FORMAT: return "invalid PSF1 or PS-X EXE";
    case PSF1_ERROR_CRC: return "compressed program CRC mismatch";
    case PSF1_ERROR_COMPRESSION: return "zlib compression error";
    case PSF1_ERROR_RANGE: return "PSF1 image is outside PlayStation RAM";
    case PSF1_ERROR_RECURSION: return "PSF library recursion limit exceeded";
    case PSF1_ERROR_MEMORY: return "out of memory";
    default: return "unknown PSF1 error";
    }
}
