...
int vab_parse_vh(const uint8_t* data, size_t len, VabHdr* hdr) {
  ...
  for (p = 1; p <= vs; p++)
    vag_offsets[p] =
        vag_offsets[p - 1] + (uint32_t)rd_u16le(data + off + p * 2) * 8;

  for (p = 0; p < ps && ti < 2048; p++) {
    for (t = 0; t < VAB_TONES_PER_PROG && ti < 2048; t++) {
      const uint8_t* tn =
          data + VAB_TONE_OFF + (p * VAB_TONES_PER_PROG + t) * VAB_TONE_SIZE;
      uint8_t vol = tn[2];
      ...
      hdr->tones[ti].program_priority = data[0x20 + (size_t)prog * 16 + 2];
...
int vab_decode_vag_ex(const uint8_t* vb, size_t vb_len, const VabHdr* hdr,
  ...
  if (loop_start)
    *loop_start = ls;
  if (loop_end)
    *loop_end = (le >= 0) ? le : (int64_t)decoded_blocks * 28;
