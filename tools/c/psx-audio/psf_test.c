...
int main(void) {
  ...
  library_exe = make_exe(0x80010000u, 0x801ffff0u, 0x80010000u, library_text,
                         sizeof(library_text), &library_size);
  mini_exe = make_exe(0x80020000u, 0x801efff0u, 0x80010001u, mini_text,
                      sizeof(mini_text), &mini_size);
  ...
  if (psf1_write_file("test.psflib", library_exe, library_size,
      ...
      psf1_write_file("test.minipsf", mini_exe, mini_size,
                      "_lib=test.psflib\n_refresh=50\n") != PSF1_OK)
    failed = 1;
  if (!failed && psf1_write_file("corrupt.psf", library_exe, library_size,
                                 NULL) == PSF1_OK) {
    int byte;
  ...
  if (!failed) {
    ...
    if (status != PSF1_OK) {
      ...
      failed = image.initial_pc != 0x80010000u ||
               image.initial_sp != 0x801ffff0u || image.refresh_rate != 50 ||
               image.loaded_min != 0x10000u || image.loaded_max != 0x10003u ||
               image.ram[0x10000] != 1 || image.ram[0x10001] != 9 ||
               image.ram[0x10002] != 8;
