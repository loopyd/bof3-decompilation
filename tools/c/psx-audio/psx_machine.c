...
static PsxMachineStatus start_spu_dma(PsxMachine* machine) {
  ...
  if (status != PSX_MACHINE_OK)
    return status;
  if (psx_spu_dma_write(machine->spu, source, (size_t)size, machine->cycles) !=
      0)
    return PSX_MACHINE_ERROR_DEVICE;
  control &= ~(1u << 24);
...
static int instruction_writes_register(uint32_t instruction, unsigned reg) {
  ...
  if (opcode == 0) {
    ...
    if (function == 0x00 || function == 0x02 || function == 0x03 ||
        ...
        (function >= 0x20 && function <= 0x27) || function == 0x2a ||
        function == 0x2b)
      return rd == reg;
    return 0;
  }
...
static PsxMachineStatus execute_special(PsxMachine* machine,
  ...
  switch (function) {
    case 0x00:
      machine->registers[rd] = machine->registers[rt] << shift;
      break;
    case 0x02:
      machine->registers[rd] = machine->registers[rt] >> shift;
      break;
    case 0x03:
      machine->registers[rd] =
          (uint32_t)((int32_t)machine->registers[rt] >> shift);
      break;
    case 0x04:
      machine->registers[rd] = machine->registers[rt]
                               << (machine->registers[rs] & 31u);
      break;
    case 0x06:
      machine->registers[rd] =
          machine->registers[rt] >> (machine->registers[rs] & 31u);
      break;
    case 0x07:
      machine->registers[rd] = (uint32_t)((int32_t)machine->registers[rt] >>
                                          (machine->registers[rs] & 31u));
      break;
    case 0x08:
    ...
    case 0x0c:
      ...
      break;
    case 0x10:
      machine->registers[rd] = machine->hi;
      break;
    case 0x11:
      machine->hi = machine->registers[rs];
      break;
    case 0x12:
      machine->registers[rd] = machine->lo;
      break;
    case 0x13:
      machine->lo = machine->registers[rs];
      break;
    case 0x18: {
    ...
    case 0x19: {
      uint64_t product =
          (uint64_t)machine->registers[rs] * (uint64_t)machine->registers[rt];
      machine->lo = (uint32_t)product;
    ...
    case 0x20:
    case 0x21:
      machine->registers[rd] = machine->registers[rs] + machine->registers[rt];
      break;
    case 0x22:
    case 0x23:
      machine->registers[rd] = machine->registers[rs] - machine->registers[rt];
    ...
    case 0x27:
      machine->registers[rd] =
          ~(machine->registers[rs] | machine->registers[rt]);
      break;
    case 0x2a:
      machine->registers[rd] =
          (int32_t)machine->registers[rs] < (int32_t)machine->registers[rt];
      break;
    case 0x2b:
      machine->registers[rd] = machine->registers[rs] < machine->registers[rt];
      break;
...
static PsxMachineStatus execute(PsxMachine* machine, uint32_t instruction) {
  ...
  switch (opcode) {
    case 0x01: {
      ...
      if (take)
        machine->next_pc =
            machine->pc + ((uint32_t)sign_extend16(immediate) << 2);
      break;
    }
    case 0x02:
      machine->next_pc =
          (machine->pc & 0xf0000000u) | ((instruction & 0x03ffffffu) << 2);
      break;
    case 0x03:
      machine->registers[31] = machine->next_pc;
      machine->next_pc =
    ...
    case 0x08:
    case 0x09:
      machine->registers[rt] =
          machine->registers[rs] + (uint32_t)sign_extend16(immediate);
      break;
    case 0x0a:
      machine->registers[rt] =
          (int32_t)machine->registers[rs] < sign_extend16(immediate);
      break;
    case 0x0b:
      machine->registers[rt] =
          machine->registers[rs] < (uint32_t)sign_extend16(immediate);
      break;
    case 0x0c:
      machine->registers[rt] = machine->registers[rs] & immediate;
      break;
    case 0x0d:
      machine->registers[rt] = machine->registers[rs] | immediate;
      break;
    case 0x0e:
      ...
      break;
    case 0x0f:
      machine->registers[rt] = immediate << 16;
      break;
    case 0x20:
    case 0x24:
      ...
      machine->pending_load_value =
          opcode == 0x20 ? (uint32_t)(int32_t)(int8_t)value8 : value8;
    ...
    case 0x25:
      ...
      machine->pending_load_register = rt;
      machine->pending_load_value =
          opcode == 0x21 ? (uint32_t)(int32_t)(int16_t)value16 : value16;
      machine->pending_load_valid = 1;
      break;
    case 0x23:
      address = machine->registers[rs] + (uint32_t)sign_extend16(immediate);
...
static PsxMachineStatus execute_bios(PsxMachine* machine) {
  ...
  if (vector == 0xa0u &&
      (function == 0x39u || function == 0x3fu || function == 0x72u)) {
    machine->pc = machine->registers[31];
    machine->next_pc = machine->pc + 4;
    return PSX_MACHINE_OK;
  }
  ...
  if (vector == 0xb0u &&
      (function == 0x18u || function == 0x19u || function == 0x3fu ||
       function == 0x5au || function == 0x5bu)) {
    machine->registers[2] = 0;
    machine->pc = machine->registers[31];
    machine->next_pc = machine->pc + 4;
    return PSX_MACHINE_OK;
...
const char* psx_machine_status_string(PsxMachineStatus status) {
  switch (status) {
    case PSX_MACHINE_OK:
      return "ok";
    case PSX_MACHINE_ERROR_ARGUMENT:
      return "invalid machine argument";
    case PSX_MACHINE_ERROR_MEMORY:
      return "out of memory";
    case PSX_MACHINE_ERROR_ADDRESS:
      return "unsupported or unaligned address";
    case PSX_MACHINE_ERROR_INSTRUCTION:
      return "unsupported instruction";
    case PSX_MACHINE_ERROR_DEVICE:
      return "device access failed";
    case PSX_MACHINE_ERROR_BIOS:
      return "unsupported BIOS call";
    case PSX_MACHINE_ERROR_LIMIT:
