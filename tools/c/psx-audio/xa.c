...
static void xa_decode_group(const uint8_t* d, int16_t* out, int sf, int16_t* p1,
                            int16_t* p2) {
  int shift = sf & 0x0F;
...
int xa_inspect(const uint8_t* data, size_t len, XaStreamInfo* streams,
               int max) {
  ...
  for (i = 0; i < nsec; i++) {
    ...
    for (j = 0; j < count; j++) {
      if (streams[j].channels == (stereo ? 2 : 1) && streams[j].rate == rate) {
        found = j;
        break;
      }
    }
    ...
    streams[found].frame_count += stereo ? XA_STEREO_SAMPLES : XA_MONO_SAMPLES;
  }
  return count;
}
    ...
    for (u = 0; u < XA_UNITS; u++) {
      ...
      if (!stereo) {
        ...
        for (g = 0; g < XA_GROUPS; g++)
          xa_decode_group(grp + g * XA_GROUP_BYTES, tmp + g * XA_GROUP_SAMPLES,
                          sf, &p1l, &p2l);
        ...
        for (g = 0; g < XA_GROUPS; g++) {
          ...
          if (g & 1)
            xa_decode_group(grp + g * XA_GROUP_BYTES,
                            tr + (g >> 1) * XA_GROUP_SAMPLES, sf, &p1r, &p2r);
          else
            xa_decode_group(grp + g * XA_GROUP_BYTES,
                            tl + (g >> 1) * XA_GROUP_SAMPLES, sf, &p1l, &p2l);
        }
