...
| Symptom | Check | Fix |
| --- | --- | --- |
| `just setup` stops before extraction | `bin/harness doctor` | Install the reported host prerequisite or place the US BIN/CUE input in `inputs/disc/`. |
...
the user-owned source media in `inputs/`.
