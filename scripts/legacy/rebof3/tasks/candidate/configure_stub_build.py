...
class ConfigureStubBuildTask(PipelineTask):
    ...
    def run(
        ...
        *,
        ...
        if (
            compile_commands_has_source(compile_commands_path, source_file=source_file)
            and not force_reconfigure(options)
        ):
            context["compile_commands_path"] = compile_commands_path
            logger.debug(f"reuse compile_commands {relative_to_root(compile_commands_path)}")
            return context
        ...
        combined_output = result.stdout + ("" if not result.stderr else "\n" + result.stderr)
        ...
        log_path = Path(str(context["workspace_json"])).resolve().parent / "configure.log"
